...
function void UNI_cell::post_randomize();
    HEC = hec(GFC, VPI, VCI, CLP, PT, Payload);
...
function bit UNI_cell::compare(input BaseTr to);
    UNI_cell cell;
    $cast(cell,to);
    if(GFC != cell.GFC) return 0;
    if(VPI != cell.VPI) return 0;
    if(VCI != cell.VCI) return 0;
    if(CLP != cell.CLP) return 0;
    if(PT != cell.PT) return 0;
    if(HEC != cell.HEC) return 0;
    if(Payload != cell.Payload) return 0;
    return 1;
endfunction : compare

//复制信元的详细内容
function void UNI_cell::copy_data(input string prefix);
    ATMCellType p;
    $display("%sUNI_cell:GFC = %0d, VPI = %0d, VCI = %0d, CLP = %0d, PT = %0d, HEC = %0d, Payload = %0d",
          prefix, GFC, VPI, VCI, CLP, PT, HEC, Payload);
    foreach(p.Mem)
    $display
endfunction : display
...
class UNI_cell_gen extends Generator;
   ...
   event drv2gen; //driver完成时的事件
   int nCells; //要产生 信元个数
   int PortID; //产生哪个Rx端口的信元
   ...
   function new(input mailbox gen2drv,
    ...
    task run();
        ...
        repeat(nCells) begin
            ...
            cell.display(%psprintf("UNI_cell_gen:UNI_cell %0d",nCells));
            gen2drv.put(cell);
            @drv2gen; //等待driver完成
        end
    endtask : run
...
class Driver;
    mailbox gen2drv; //用于存储器发送的信元
    event drv2gen; //通知发生器已经处理完毕
    vUtopiaRx Rx; //发送信元的虚IFC
...
task Monitor::run();
    ...
    task Monitor::receive(output NNI_cell cell);
        ATMCellType pkt;
        Tx.cbr.clav <= 1;
        while(Tx.cbr.soc !== 1'b1 && Tx.cbt.en !== 1'b0)
            @(Tx.cbr);
        for(int i = 0; i <= 52; i++) begin
            //如果没有使能,循环等待
            while(Tx.cbt.en !== 1'b0) @(Tx.cbr);
        ...
        cell = new();
        cell.unpack(Pkt);
        cell.display(%psprintf("Monitor:NNI_cell %0d",PortID));
    endtask:receive
```

计分板scoreboard通过save_expected函数从Driver获得期望的信元,信元实际上从监视器monitor的check_actual函数接收,save_expected函数回调任务Scb_Driver——cbs
...
class Scoreboard;
    ...
    extern  function void save_expected(UNI_cell cell);
...
function void Scoreboard::save_expected(UNI_cell cell);
    NNI_cell ncell = ucell.to_NNI;
    CellCfgType CellCfg = top.squat.lut.read(ncell.VPI);

    $display("@%0d Scoreboard:UNI_cell %0d, VPI = %0d, VCI = %0d, CLP = %0d, PT = %0d, HEC = %0d, Payload = %0d",
             $time, iexpect, cell.VPI, cell.VCI, cell.CLP, cell.PT, cell.HEC, cell.Payload);
    $time,ncell,VPI,CellCfg.FWD;
    ncell.display("psprintf("@ 0t:Scb save :", $time));

    //寻找信元将要转发的Tx端口
    for(int i = 0 ; i < NumTx ; i++)
        if(CellCfg.FWD[i].q.push_back(ncell)); //把信元保存到q
            expect_cells[i].q.push_back(ncell); //把信元保存到q
            expect_cells[i].iexpect++;
            iexpect++;
        end
    endfunction : save_expected

    function void Scoreboard::check_actual(input NNI_cell cell,input int portn);
        NNI_cell match;
        int  match_idx;

        cell.display($psprintf("@%0t:Scb check:", $time));
        if(expect_cells[portn].q.size() == 0) begin
            $display("@%0t:Scb check:No expected cell for port %0d", $time, portn);
            return;
        end
        ...
        foreach(expect_cells[portn].q[i])begin
            if(expect_cells[portn].q[i].compare(cell)) begin
                $display("@%0t:Scb check:Matched expected cell %0d", $time, i);
                expect_cells[portn].q.delete(i);
                return;
            end
        end
        $display("@%0t:Scb check:No match for expected cell %0d", $time, portn);
    ...
    function void Scoreboard::wrap_up();
        $display("Scoreboard:Total expected cells = %0d, Total actual cells = %0d", iexpect, iactual);
        //寻找剩余的信元
        foreach (expect_cells[i]) begin
            if(expect_cells[i].q.size() != 0) begin
                $display("@%0t:Scb wrap up:Port %0d has %0d unmatched expected cells", $time, i, expect_cells[i].q.size())
...
function void Scoreboard::display(string prefix);
    $display("@%0t:Scb display: %s", $time, prefix);
    foreach(expect_cells[i]) begin
        $display("@%0t:Scb display: Port %0d", $time, i);
        foreach(expect_cells[i].q[j]) begin
...
$display("Coverage:Src = %0d, Fwd = %0d", src, fwd);
this.src = src;
this.fwd = fwd;
CG_Forward.sample();
endfunction : sample
...
task CPU_driver::run();
    ...
    `endif
        CellFwd.VPI = i;
        HostRead(i,CellFwd);
        if(lookup[i].FWD != CellFwd.FWD) begin
            $display("Error: Memory verification failed for VPI = %0d", i);
        $finish;
    end
end
...
在验证期间,根据要测试的功能建立很多测试集,每个测试集使用不同的种子来运行
...
program automatic test
    ...
    initial begin
        ...
        begin //故障注入
            Driver_cbs_drop dcd = new();
            env.drv.cbs.push_back(dcd); //放入driver的队列
        end
...
import 声明定义了C任务和函数的原型,但使用的是SystemVerilog的数据类型

带有返回值的C函数会被映射成一个SystemVerilog函数
...
program automatic test;
    ...
    // C 函数与关键字同名，需要修改函数名
    // 注意：原图中此处使用了转义符 \expect=，但在标准语法中通常直接使用 expect=
...
| bit | svBit or<br>unsigned char | svBit* or<br>unsigned char |
| logic, reg | svLogic or<br>unsigned char | svLogic* or<br>unsigned char* |
| bit[N:0]<br>reg[N:0]<br>logic[N:0] | const svBitVecVal*<br>const svLogicVecVal* | svBitVecVal*<br>svLogicVecVal* |
| open array[] | const svOpenArrayHandle | svOpenArrayHandle |
| chandle | const void* | void* |

chandle 数据类型

该数据类型允许你在SystemVerilog代码中存储一个C/C++指针,一个chandle变量的宽度足够在其被编译的机器上保存一个指针变量
...
verilog代码的数值是4状态所以用2位表示，c中高位aval，低位bval

```c
// 例 12.12 检查 X 和 Z 值的计数器程序
// 将例 12.9 中的 counter7 替换为四态变量
...
void counter7(c7 * inst,
    ...
    if (reset)
        ...
        inst->cnt = i->aval;          // 加载数值
    else
        inst->cnt++;                  // 计数

    inst->cnt &= 0x7F;                // 最高位清 0 (保持 7-bit)

    // 赋值给输出变量
    count->aval = inst->cnt;
    count->bval = 0;                  // 确保输出为确定的双态值 (0 或 1)
}
```
...
void Counter7::counter7_signal(svBitVecVal* count,
    ...
    cnt &= 0x7F;                  // 最高位清 0 (保持7位宽度)
    *count = cnt;
}
```

DPI只能调用静态的C或者CPP代码,即在链接时已经存在的方法,这样一来,SystemVerilog代码就不能调用对象中的Cpp方法,因为被调用的对象在目标代码连接器运行时还不存在

解决方法是创建可以与CPP动态对象和方法通信的静态方法
...
program automatic counter;
    ...
    initial begin
        ...
        $display("SV : Post reset : counter1 = %d",c1.get());
...
```c
import "DPI-C" function void fib(svBitVecVal data[20]);
...
| `int svIncrement(h, d)` | 如果左边界大于等于右边界则返回 1，<br>如果左边界小于右边界则返回 -1 |
...
#include <stdio.h>
#include "svdpi.h" // 包含 svOpenArrayHandle 等定义
...
在dpi中一个开放数组被视为拥有一个压缩维度和一个或多个非压缩的维度

压缩开放数组的测试平台

在 DPI 中，一个开放数组被视为拥有一个压缩的维度和一个或多个非压缩的维度。你可以传递多维的压缩数组，只要该压缩数组的元素大小跟形式参数的元素大小相同即可。例如，如果在 `import` 语句中定义了形式参数 `bit [63:0] b64[]`，就可以将实际参数 `bit[1:0][0:3][6:-1] bpack[9:1]` 传递进去。
...
注意，例 12.28 中的 C 代码按照 `%llx` 的格式输出一个 64 位数值，然后将结果从 `svGetArrayElemPtr1` 类型强制转换成了 `long long int` 类型。

---

这个例子展示了如何传递“打包”数组，需要我帮你写个对应的 SystemVerilog 接口来调用它吗？

### 共享符合类型

下面是一个共享用于表示像素的简单结构变量的例子，该结构由三个字节组成，被压缩成一个字。例 12.29 给出了 C 结构的定义。注意，C 将 `char` 视为有符号变量，这可能会带来不可预测的结果，所以该结构对 `char` 类型加上 `unsigned` 限制。这些字节的顺序跟 SystemVerilog 相比刚好相反，因为这段代码运行在小尾序的 Intel X86 处理器上，即低权重字节存储在低位地址上。Sun SPARC 处理器是大尾序的，所以其字节存储顺序跟 SystemVerilog 相同：r, g, b。
...
char * print(p_rgb rgb){
    ...
    nchars = sprintf(s,"C: rgb=%02x,%02x,%02x",rgb->r,rgb->g,rgb->b);
...
导入方法分为纯（pure）导入、关联（context）导入和通用（generic）导入。一个纯函数将严格根据其输入来计算输出，跟外部环境没有任何其他交互。具体来说就是一个纯函数不会访问任何全局或者静态变量，不会进行文件操作，不会跟函数体以外的事务如操作系统、进程、共享内存和套接字等有交互。如果没有使用纯函数的输出，那么 SystemVerilog 编译器会优化掉对该函数的调用，对于输入参数相同的两次调用，编译器会将第二次调用直接用第一次的输出结果替换。例 12.5 中的 `factorial` 函数和例 12.6 中的 `sin` 函数都是纯函数，因为它们的计算结果仅依赖于它们的输入。
...
导入方法可能需要知道它被调用的环境的上下文信息以决定调用 PLI TF、ACC 还是 VPI 方法或者导出的 SystemVerilog 任务。对这些导入方法需要使用 `context` 限定词。

```systemverilog
import "DPI-C" context task call_sv(bit [31:0] data);
```

如果导入方法使用全局变量，那它就不再是纯方法了，但是它可能没有调用任何 PLI，所以其实它也不需要承受 `context` 方法所带来的额外开销。Sutherland (2004) 将这样的函数定义为“generic（通用的）”，因为 SystemVerilog 语言参考手册中对此并没有定义专门的名字。缺省情况下导入函数是通用类型，这也是本章中的很多例子使用的类型。

调用关联导入方法时需要记录调用的上下文环境，这会给仿真器带来额外的开支。所以除非确实需要，否则不要将方法定义为关联方法。另一方面，如果一个通用导入方法调用了一个导出任务或者一个访问 SystemVerilog 数据对象的 PLI 函数，会导致仿真器崩溃。

一个关联感知（context-aware）的 PLI 函数指的是这个函数知道自己身在何处被调用，并且能够依据调用地址来访问跟该位置相关联的信息。
